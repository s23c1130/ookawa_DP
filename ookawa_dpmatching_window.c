...
int main(void)
{
    ...
    for (int input_data_cnt = 0; input_data_cnt < DATASET_NUM_MAX; input_data_cnt++)
    {
        ...
        for (int i = 0; i < DATASET_NUM_MAX; i++)
        {
            ...
            int dataset_cnt_start = (-1) * WINDOW - 1;
            int dataset_cnt_end = WINDOW - 1;
            ...
            for (int input_cnt = 0; input_cnt < input_frame[input_data_cnt]; input_cnt++)
            {
                ...
                int dataset_cnt_start_tmp = dataset_cnt_start;

                if(dataset_cnt_start_tmp < 0){
                    dataset_cnt_start_tmp = 0;
                }

                for (int dataset_cnt = dataset_cnt_start_tmp; dataset_cnt < dataset_frame[i]; dataset_cnt++)
                {
                    if(dataset_cnt >= dataset_cnt_end){
                        break;
                    }
                    ...
                    if (input_cnt >= 1 && dataset_cnt < dataset_cnt_end - 1)
                    {
                    ...
                    if (dataset_cnt >= 1 && dataset_cnt > dataset_cnt_start)
                    {
